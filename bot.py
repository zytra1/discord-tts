...
bot = bridge.Bot(
    ...
    description=
    "A bot to convert text to speech in a voice channel using Google TTS APIs.",
    intents=intents,
...
def return_language_string(lang, string):
    ...
    try:
        ...
        print(
            f"-> String '{string}' not found in language '{lang}', defaulting to 'en'"
        )
        lang = "en"
        config.read(os.path.join(langfolder, lang), encoding='utf-8')
    ...
    for guild in bot.guilds:
        ...
        if not discord.utils.get(guild.roles, name=config['DEFAULT']['role']):
            try:
                ...
                print(
                    f"-> Role '{config['DEFAULT']['role']}' created in {guild.name}"
                )
            except:
                print(
                    f"-> Role '{config['DEFAULT']['role']}' could not be created in {guild.name}"
                )
        else:
            print(
                f"-> Role '{config['DEFAULT']['role']}' found in {guild.name}")
...
async def check_role(ctx):
    ...
    try:
        ...
        config.read(os.path.join(configs, str(ctx.guild.id)), encoding='utf-8')
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'errtitle')),
            description=eval("f" + get_guild_language(ctx, 'errnorole')),
            color=0xFF0000)
        embed.set_thumbnail(
            url=
    ...
    if role in ctx.author.roles:
        ...
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'errtitle')),
            description=eval("f" + get_guild_language(ctx, 'errrole')),
            color=0xFF0000)
        embed.set_thumbnail(
            url=
            "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8e/Antu_dialog-error.svg/1024px-Antu_dialog-error.svg.png"
            ...
            if ctx.author.voice:
                if voice.channel == ctx.author.voice.channel:
                    ...
                    embed = discord.Embed(
                        title=eval("f" + get_guild_language(ctx, 'errtitle')),
                        description=eval("f" +
                                         get_guild_language(ctx, 'errvoice')),
                        color=0xFF0000)
                    await ctx.respond(embed=embed, delete_after=5)
                    return False
            else:
                embed = discord.Embed(
                    title=eval("f" + get_guild_language(ctx, 'errtitle')),
                    description=eval("f" + get_guild_language(ctx, 'erruvc')),
                    color=0xFF0000)
                await ctx.respond(embed=embed, delete_after=5)
                return False
        else:
        ...
        if ctx.author.voice:
            ...
            try:
                ...
                embed = discord.Embed(
                    title=eval("f" + get_guild_language(ctx, 'errtitle')),
                    description=eval(
                        "f" + get_guild_language(ctx, 'errcantconnect')),
                    color=0xFF0000)
                await ctx.respond(embed=embed, delete_after=5)
                return False
            addqueue(source, ctx.guild)
        ...
        if voice:
            if voice.is_connected():
                ...
                voice.play(discord.FFmpegPCMAudio(source),
                           after=lambda e: nextqueue(guild, channelid))
            else:
                resetqueue(guild.id)
        else:
            resetqueue(guild.id)
...
async def check_update():
    try:
        r = requests.get(
            'https://api.github.com/repos/CoccodrillooXDS/TTS-bot/releases/latest',
            timeout=5)
        if r.status_code == 200:
            with open(os.path.join(root, 'version.ini'), 'r') as f:
                version = f.read()
            if int(version) > r.json()['id'] and bot_version != r.json(
            )['tag_name']:
                print(
                    f"-> Bot version ({bot_version}) is newer than the latest version ({r.json()['tag_name']})"
                )
                return
            if bot_version != r.json()['tag_name'] and int(
                    version) == r.json()['id']:
                print(
                    f"-> Bot version ({bot_version}) is newer than the latest version ({r.json()['tag_name']})"
                )
            ...
            if int(version) != r.json()['id'] and bot_version == r.json(
                ...
                id = r.json()['id']
                with open(os.path.join(root, 'version.ini'),
                          'w') as versionfile:
                ...
                if silent:
                    print(
                        f"-> A new silent version ({bot_version}) has been installed successfully!"
                    )
                    return
                else:
                    print(
                        f"-> A new version ({bot_version}) has been installed successfully!"
                    )
                    for guild in bot.guilds:
                        ctx = guild
                        config = configparser.ConfigParser()
                        ...
                        if config['DEFAULT']['silenceupdates'] == "True":
                            print(
                                f"!> Update message not sent to {guild.name} ({guild.id}). Updates have been silenced"
                            )
                            continue
                        embed = discord.Embed(
                            title=eval("f" +
                                       get_guild_language(ctx, 'updatetitle')),
                            description=eval(
                                "f" + get_guild_language(ctx, 'updatedesc')),
                            color=0x286fad)
                        try:
                            if config['DEFAULT']['updateschannel'] == "system":
                                await guild.get_channel(guild.system_channel.id
                                                        ).send(embed=embed)
                                print(
                                    f">> Update message sent to {guild.name} ({guild.id}) in system channel {guild.system_channel.name}."
                                )
                            else:
                                await guild.get_channel(
                                    int(config['DEFAULT']['updateschannel'])
                                ).send(embed=embed)
                                print(
                                    f">> Update message sent to {guild.name} ({guild.id}) in {guild.get_channel(int(config['DEFAULT']['updateschannel'])).name}."
                                )
                        except:
                            try:
                                for channel in guild.text_channels:
                                    if channel.permissions_for(
                                        ...
                                        print(
                                            f">> Update message sent to {guild.name} ({guild.id}) in {channel.name}. << Unable to find the channel specified in the config file."
                                        )
                                        break
                            except:
                                print(
                                    f"!> Update message not sent to {guild.name} ({guild.id}). << Unable to find a channel where I can send messages."
                                )
                                continue
...
async def langs(ctx):
    ...
    embed = discord.Embed(title=eval("f" + get_guild_language(ctx, 'avlang')),
                          description=supported_languages_message,
                          color=0x37ff00)
...
async def say(ctx, lang: Option(str,
                                "Choose a language",
                                autocomplete=showlangs),
              text: Option(str, "Enter text to convert to speech")):
    global temp
    await ctx.defer()
    config = configparser.ConfigParser()
    ...
    if time1:
        for x in time1:
            ...
            texta = re.sub(r"<t:({})>".format(x),
                           t.strftime("%A %d %B %Y, %H:%M:%S"), texta)
    time2 = re.findall(r"<t:(\d+):(\w+)>", texta)
    if time2:
        for x in time2:
            t = datetime.datetime.fromtimestamp(int(x[0]))
            texta = re.sub(r"<t:({}):({})>".format(x[0], x[1]),
                           t.strftime("%A %d %B %Y, %H:%M:%S"), texta)
    users = []
    if os.path.isfile(os.path.join(temp, str(ctx.guild.id), ".users")):
        ...
        if not author in users:
            users.append(author)
            with open(os.path.join(temp, str(ctx.guild.id), ".users"),
                      "wb") as f:
                pickle.dump(users, f)
        if len(users) > 1:
    ...
    try:
        ...
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'errtitle')),
            description=eval("f" + get_guild_language(ctx, 'unexpectederror')),
            color=0xFF0000)
        await ctx.respond(embed=embed, delete_after=5)
        return
    if await preplay(ctx, source):
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'done')),
            description=eval("f" + get_guild_language(ctx, 'saylangmess')),
            color=0x1eff00)
        await ctx.respond(
            embed=embed,
...
async def hidsay(ctx, lang, text):
    ...
    if texta == "" or texta.isspace():
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'errtitle')),
            description=eval("f" + get_guild_language(ctx, 'errnoarg')),
            color=0xFF0000)
        await ctx.respond(embed=embed, delete_after=5)
        return
    user = re.findall(r"<@!?(\d+)>", texta)
...
@bot.slash_command(name="stop",
                   description="Disconnect the bot from current channel",
                   default_permissions=False)
@commands.check(check_role)
async def _stop(ctx):
    await stop(ctx)
...
async def stop(ctx):
    if ctx.voice_client:
        ...
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'done')),
            description=eval("f" + get_guild_language(ctx, 'disconnected')),
            color=0x1eff00)
        await ctx.respond(embed=embed, delete_after=3)
    else:
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'errtitle')),
            description=eval("f" + get_guild_language(ctx, 'errnovc')),
            color=0xFF0000)
        await ctx.respond(embed=embed, delete_after=3)
...
class Help(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        embed = discord.Embed(
            title=eval("f" + get_guild_language(self.context, 'helptitle')),
            description=allhelp,
            color=0x1eff00)
...
async def help(ctx):
    ...
    embed = discord.Embed(title=eval("f" +
                                     get_guild_language(ctx, 'helptitle')),
                          description=allhelp,
                          color=0x1eff00)
    await ctx.respond(embed=embed)
...
async def about(ctx):
    ...
    embed = discord.Embed(
        title=eval("f" + get_guild_language(ctx, 'abouttitle')),
        description=eval("f" + get_guild_language(ctx, 'aboutdesc')),
        color=0x1eff00)
    await ctx.respond(embed=embed)
...
class setrole(Modal):
    ...
    def __init__(self, ctx, *args, **kwargs) -> None:
        ...
        self.add_item(
            InputText(custom_id="role",
                      label=eval("f" +
                                 get_guild_language(ctx, 'changerolemodal')),
                      placeholder="TTS",
                      value=""))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        config.read(os.path.join(configs, str(interaction.guild.id)),
                    encoding='utf-8')
        newrole = discord.utils.get(interaction.guild.roles,
        ...
        errorembed = discord.Embed(
            title=eval("f" + get_guild_language(interaction, 'errtitle')),
            description=eval(
                "f" + get_guild_language(interaction, 'unexpectederrorrole')),
            color=0xff0000)
        warningremoveembed = discord.Embed(
            title=eval("f" + get_guild_language(interaction, 'done')),
            description=warnrem,
            color=0xf0e407)
        warningeditembed = discord.Embed(
            title=eval("f" + get_guild_language(interaction, 'done')),
            description=warnedit,
            color=0xf0e407)
        embed = discord.Embed(
            title=eval("f" + get_guild_language(interaction, 'done')),
            description=eval("f" +
                             get_guild_language(interaction, 'rolechange')),
            color=0x1eff00)
        if oldrole is None and newrole is None:
            try:
                ...
                print(
                    f"{interaction.guild.name}: Created role {self.children[0].value}"
                )
            except:
                e = traceback.format_exc()
                print(
                    f"Error: Could not create role {self.children[0].value} in {interaction.guild.name}"
                )
                print(e + "Error Code: " + code)
        ...
        if oldrole is not None and newrole is None:
            try:
                ...
                print(
                    f"{interaction.guild.name}: Old role {oldroleconf} renamed to {self.children[0].value}"
                )
                newrole = discord.utils.get(interaction.guild.roles,
                                            name=self.children[0].value)
            except:
                e = traceback.format_exc()
                print(
                    f"Error: Could not edit role name from {oldroleconf} to {self.children[0].value} in {interaction.guild.name}\{e}"
                )
                ...
                try:
                    await interaction.guild.create_role(
                        name=self.children[0].value)
                    print(
                        f"{interaction.guild.name}: Warning: Created new role (unable to edit the old role) {self.children[0].value}"
                    )
                    newrole = discord.utils.get(interaction.guild.roles,
                                                name=self.children[0].value)
                except:
                    e = traceback.format_exc()
                    print(
                        f"Error: Could not edit {oldroleconf} and could not create role {self.children[0].value} in {interaction.guild.name}"
                    )
                    print(e + "Error Code: " + code)
                    await interaction.response.send_message(embed=errorembed,
                                                            delete_after=10,
        ...
        if newrole:
            if newrole.id not in interaction.user.roles:
                try:
                    ...
                    print(
                        f"{interaction.guild.name}: {interaction.user.name} has been given {newrole.name}"
                    )
                except:
                    e = traceback.format_exc()
                    print(
                        f"Error: Could not add the role {newrole.name} to {interaction.user.name}"
                    )
                    print(e + "Error Code: " + code)
                    await interaction.response.send_message(embed=errorembed,
                                                            delete_after=10,
        ...
        if oldrole is not None and newrole is not None:
            try:
                ...
                e = traceback.format_exc()
                print(
                    f"Error: Could not delete old role from {interaction.guild.name}\n{e}"
                )
                embed = warningremoveembed
        config['DEFAULT']['role'] = self.children[0].value
        with open(os.path.join(configs, str(interaction.guild.id)),
                  'w') as configfile:
            config.write(configfile)
...
async def _settings(ctx, context):
    ...
    usci = 0
    if config["DEFAULT"]["updateschannel"] == "system" or config["DEFAULT"][
            "updateschannel"] == "":
        try:
    ...
    embed = discord.Embed(
        title=eval("f" + get_guild_language(ctx, 'currsettingstitle')),
        description=eval("f" + get_guild_language(ctx, 'currsettings')),
        color=0x1eff00)
    buttonclose = Button(custom_id="close",
                         label=eval("f" + get_guild_language(ctx, 'close')),
                         style=discord.ButtonStyle.danger)
    buttonchangelanguage = Button(
        custom_id="cl",
        label=eval("f" + get_guild_language(ctx, 'changelang')),
        style=discord.ButtonStyle.secondary,
        emoji="🏴‍☠️")
    buttonchangerole = Button(
        custom_id="cr",
        label=eval("f" + get_guild_language(ctx, 'changerole')),
        style=discord.ButtonStyle.secondary,
        emoji="🏷️")
    buttondefvoice = Button(
        custom_id="cdv",
        label=eval("f" + get_guild_language(ctx, 'changedefaultvoice')),
        style=discord.ButtonStyle.secondary,
        emoji="🗣️")
    buttonautosaychan = Button(
        custom_id="casc",
        label=eval("f" + get_guild_language(ctx, 'changeautosaychannel')),
        style=discord.ButtonStyle.secondary,
        emoji="#️⃣")
    buttonchangeupdatechannel = Button(
        custom_id="cuc",
        label=eval("f" + get_guild_language(ctx, 'changeupdatechannel')),
        style=discord.ButtonStyle.secondary,
        emoji="🗣️")
    buttonsilenceupdates = Button(
        custom_id="su",
        label=eval("f" + get_guild_language(ctx, 'silenceupdates')),
        style=discord.ButtonStyle.secondary,
        emoji="🔇")
    buttonmultiuser = Button(custom_id="mu",
                             label=eval("f" +
                                        get_guild_language(ctx, 'multiuser')),
                             style=discord.ButtonStyle.secondary,
                             emoji="👥")
    buttonusenicknames = Button(
        custom_id="un",
        label=eval("f" + get_guild_language(ctx, 'usenicknames')),
        style=discord.ButtonStyle.secondary,
        emoji="🏷️")
    view = View()
    ...
    async def defvoice(ctx):
        ...
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'changedefaultvoice')),
            description=eval(
                "f" + get_guild_language(ctx, 'changedefaultvoicedesc')),
            color=0x1eff00)
        options1 = []
        options2 = []
        options3 = []
        ...
        for i in lang_list:
            ...
            elif len(options1) == 25 and len(options2) == 25 and len(
                    options3) < 25:
                options3.append(discord.SelectOption(label=i, value=i))
            elif len(options1) == 25 and len(options2) == 25 and len(
                    options3) == 25 and len(options4) < 25:
        ...
        async def callback(ctx):
            ...
            try:
                ...
                pass
            with open(os.path.join(configs, str(ctx.guild.id)),
                      'w') as configfile:
                config.write(configfile)
            embed = discord.Embed(title=eval("f" +
                                             get_guild_language(ctx, 'done')),
                                  color=0x1eff00)
        ...
        if len(options1) > 0:
            select = Select(custom_id="defvoice",
                            max_values=1,
                            options=options1)
            view.add_item(select)
            select.callback = callback
        if len(options2) > 0:
            select2 = Select(custom_id="defvoice2",
                             max_values=1,
                             options=options2)
            view.add_item(select2)
            select2.callback = callback
        if len(options3) > 0:
            select3 = Select(custom_id="defvoice3",
                             max_values=1,
                             options=options3)
            view.add_item(select3)
            select3.callback = callback
        if len(options4) > 0:
            select4 = Select(custom_id="defvoice4",
                             max_values=1,
                             options=options4)
    ...
    async def autosaychan(ctx):
        ...
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'changeautosaychannel')),
            color=0x1eff00)
        options = []
        options2 = []
        options3 = []
        ...
        for i in ctx.guild.text_channels:
            if len(options) < 25:
                if int(i.id) in ascj:
                    options.append(
                        discord.SelectOption(label=i.name,
                                             value=str(i.id),
                                             default=True))
                    optsel.append(i.id)
                else:
                    options.append(
                        discord.SelectOption(label=i.name, value=str(i.id)))
                ...
                if int(i.id) in ascj:
                    ...
                    options2.append(
                        discord.SelectOption(label=i.name, value=str(i.id)))
            elif len(options) == 25 and len(options2) == 25 and len(
                    options3) < 25:
                if int(i.id) in ascj:
                    options3.append(
                        discord.SelectOption(label=i.name,
        ...
        if lo1 > 25:
            lo1 = 25
        if not lo1 == 0:
            select = Select(custom_id="autosaychan",
                            min_values=0,
                            max_values=lo1,
                            options=options)
            view.add_item(select)
        ...
        if lo2 > 25:
            lo2 = 25
        if not lo2 == 0:
            select2 = Select(custom_id="autosaychan2",
                             min_values=0,
                             max_values=lo2,
                             options=options2)
            view.add_item(select2)
        ...
        if lo3 > 25:
            lo3 = 25
        if not lo3 == 0:
            select3 = Select(custom_id="autosaychan3",
                             min_values=0,
                             max_values=lo3,
                             options=options3)
            view.add_item(select3)
            select3.callback = onlysel
        buttonapply = Button(custom_id="apply",
                             label=eval("f" +
                                        get_guild_language(ctx, 'apply')),
        ...
        if optsel == [] and optsel2 == [] and optsel3 == []:
            buttondisable = Button(
                custom_id="disable",
                disabled=True,
                label=eval("f" + get_guild_language(ctx, 'disable')),
                style=discord.ButtonStyle.danger)
        else:
            buttondisable = Button(
                custom_id="disable",
        ...
        async def disable(ctx):
            ...
            with open(os.path.join(configs, str(ctx.guild.id)),
                ...
                config.write(configfile)
            embed = discord.Embed(
                title=eval("f" + get_guild_language(ctx, 'disabledautosay')),
                color=0xe32222)
            await ctx.message.delete()
    ...
    async def changerole(ctx):
        ...
        modal = setrole(ctx,
                        title=eval("f" +
                                   get_guild_language(ctx, 'changerole')))
    ...
    async def changelanguage(ctx):
        ...
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'changelang')),
            description=eval("f" + get_guild_language(ctx, 'changelangdrop')),
            color=0x1eff00)
        options = []
        for i in installed_langs:
            options.append(discord.SelectOption(label=i, value=i))
        select = Select(custom_id="selecta",
                        max_values=1,
        ...
        async def callback(ctx):
            ...
            embed = discord.Embed(
                title=eval("f" + get_guild_language(ctx, 'done')),
                description=eval("f" + get_guild_language(ctx, 'changedlang')),
                color=0x1eff00)
    ...
    async def changeupdatechannel(ctx):
        ...
        embed = discord.Embed(
            title=eval("f" + get_guild_language(ctx, 'changeupdatechannel')),
            color=0x1eff00)
        options1 = []
        options2 = []
        options3 = []
        ...
        if len(options1) > 0:
            select = Select(custom_id="updateschannel",
                            max_values=1,
                            options=options1)
            view.add_item(select)
            select.callback = callback
        if len(options2) > 0:
            select2 = Select(custom_id="updateschannel2",
                             max_values=1,
                             options=options2)
            view.add_item(select2)
            select2.callback = callback
        if len(options3) > 0:
            select3 = Select(custom_id="updateschannel3",
                             max_values=1,
                             options=options3)
            view.add_item(select3)
            select3.callback = callback
        if len(options4) > 0:
            select4 = Select(custom_id="updateschannel4",
                             max_values=1,
                             options=options4)
            view.add_item(select4)
            select4.callback = callback
    ...
    async def silenceupdates(ctx):
        ...
        if config["DEFAULT"]["silenceupdates"] == "True":
            ...
            embed = discord.Embed(
                title=eval("f" + get_guild_language(ctx, 'done')),
                description=eval("f" +
                                 get_guild_language(ctx, 'unsilencedupdates')),
                color=0x1eff00)
        else:
            ...
            embed = discord.Embed(
                title=eval("f" + get_guild_language(ctx, 'done')),
                description=eval("f" +
                                 get_guild_language(ctx, 'silencedupdates')),
                color=0x1eff00)
    ...
    async def multiuser(ctx):
        ...
        if config["DEFAULT"]["multiuser"] == "True":
            ...
            embed = discord.Embed(
                title=eval("f" + get_guild_language(ctx, 'done')),
                description=eval("f" +
                                 get_guild_language(ctx, 'singleusermsg')),
                color=0x1eff00)
        else:
            ...
            embed = discord.Embed(
                title=eval("f" + get_guild_language(ctx, 'done')),
                description=eval("f" +
                                 get_guild_language(ctx, 'multiusermsg')),
                color=0x1eff00)
        with open(os.path.join(configs, str(ctx.guild.id)), 'w') as configfile:
            config.write(configfile)
        await ctx.message.delete()
    ...
    async def usenicknames(ctx):
        ...
        if config["DEFAULT"]["usenicknames"] == "True":
            ...
            embed = discord.Embed(
                title=eval("f" + get_guild_language(ctx, 'done')),
                description=eval("f" +
                                 get_guild_language(ctx, 'usingusernames')),
                color=0x1eff00)
        else:
            ...
            embed = discord.Embed(
                title=eval("f" + get_guild_language(ctx, 'done')),
                description=eval("f" +
                                 get_guild_language(ctx, 'usingnicknames')),
                color=0x1eff00)
        with open(os.path.join(configs, str(ctx.guild.id)), 'w') as configfile:
            config.write(configfile)
        await ctx.message.delete()
            ...
            if time.time() - float(config['DEFAULT']['time']) >= 900:
                if guild.voice_client is not None:
                    try:
                        ...
                        print(
                            f"-> Disconnected for inactivity from '{guild.name}'"
                        )
                    except AttributeError:
                        pass
...
async def on_ready():
    ...
    for guild in bot.guilds:
        if not os.path.exists(os.path.join(configs, str(guild.id))):
            ...
            if not "DEFAULT" in config:
                config['DEFAULT'] = conf
                with open(os.path.join(configs, str(guild.id)),
                          'w') as configfile:
                    config.write(configfile)
            for key in conf:
    ...
    an += 1
    print(
        f"Checking if all servers have the role specified in config... (Step {str(an)}/{str(n)})"
    )
    try:
        await loadroles(bot)
        ...
        print(
            f"An error occurred when loading roles (Step {str(an)}/{str(n)})")
    ...
    if not discord.utils.get(guild.roles, name="TTS"):
        try:
            ...
            embed = discord.Embed(
                title=f"{guild.name}",
                description=
                f"**Hi!**\nThank you for adding me to '_**{guild.name}**_'!\n\n**AN ERROR OCCURRED**:\nI can't create a role called '**TTS**' (it allows the users to use this bot).\n\n**WARNING**: Without this role you **CAN'T** use the bot.\n\nAfter creating manually a role called '**TTS**' you can change bot settings with the **`/settings`** command",
                color=0x286fad)
    try:
...
if os.environ.get('TOKEN') is None:
    print(
        "Bot TOKEN not found. You can get one by creating a bot from https://discord.com/developers/applications/. Please set the TOKEN environment variable."
    )
    exit()
else:
